% Alkishriwo example of TDCH and iTDCH (modified)
function [X] = chirp_Parameter_Estimation (testCase)
...
% ARG- the below was the original equation but I think there was an error
% in the chirp oparts of the two components
%x=(1*exp(j*(2*0.2*pi*(n).^2/(1*M)+2*pi*25*(n)/M+pi/2)))+1*exp(j*(-2*pi*0.1*n.^2/(1*M)+2*pi*225*n/M+pi/4));

% ARG -so I broke it apart into two components so I could look at each one
% in a time domain plot.
%x1 = 1*exp(j*(2*0.2*pi*(n).^2/(1*M)+2*pi*25*(n)/M+pi/2));
%x2 = 1*exp(j*(-2*pi*0.1*n.^2/(1*M)+2*pi*225*n/M+pi/4));

% ARG - I believe the below is the intended signal
x1 = 1*exp(j*(2*0.2*pi*(n/M).^2+2*pi*25*(n)/M+pi/2));
x2 = 1*exp(j*(-2*pi*0.1*(n/M).^2+2*pi*225*(n)/M+pi/4));
x = x1 + x2;

%-----------------------debug look at the signal ------------------------------------
%X = x;
% plot(X);
%--------------------------------------------------------------------------
...
% noise is not needed for our understanding
x1 = x;
